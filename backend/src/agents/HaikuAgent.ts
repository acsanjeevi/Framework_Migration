...
/**
 * Wraps Claude Haiku 4.5 for code transformation calls.
 * Cost: 0.33× relative to Sonnet — used as the primary agent.
 */
export class HaikuAgent {
 ...
 * Measures overlap of identifiers between source and migrated output.
 *
 * Formula weight matches architecture spec:
 *   selectors 0.40 + methods 0.30 + assertions 0.20 + pattern fidelity 0.10
 */
function computeHeuristicConfidence(source: string, migrated: string): number {
  ...
  // Pattern fidelity: check that Playwright test() or describe() is present
